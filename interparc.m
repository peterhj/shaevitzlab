function pt = interparc(t,px,py,varargin)
...
if nargin > 3
  ...
  if ischar(varargin{end})
    ...
    % method may be any of 'linear', 'pchip', 'spline'
    % any any simple contraction thereof
    valid = {'linear' 'pchip' 'spline'};
...
% or pchip interpolant to work with.
...
if (numel(cumarc) == 3) && strcmp(method,'spline')
  cumarc = spl{1}.breaks;
  n = numel(cumarc);
...
seglen = zeros(n-1,1);
% totalsplinelength is defeined as 1 here,
% but it allows us later to normalize the
% spline to have a unit arc length.
totalsplinelength = 1;
% options for ode45
...
% catch any problems at the ends
tbins((tbins <= 0) | (t <= 0)) = 1;
tbins((tbins >= n) | (t >= totalsplinelength)) = n - 1;
